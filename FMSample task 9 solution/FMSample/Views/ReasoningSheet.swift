...
struct ReasoningSheet: View {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 16) {
                            Text(reasoningOutput)
                                .padding()
                            if let reasoningSession, generationState == .completed {
                                HStack {
                                    Spacer()
                                    FeedbackButtons(feedbackState: $feedbackState, session: reasoningSession)
                                        .padding()
                                }
                            }
                        }
    ...
    private func loadActionItemsContent() async throws -> LanguageModelSession {
        ...
        let session = LanguageModelSession(model: model, instructions: "You are a helpful assistant. Your task is to extract action items from transcripts of team meetings. The text provided to you contains team meeting transcripts. You MUST return a list of priority action items based ONLY on the provided text.")
...
//            .filter {$0 is RecordingItem}
    ...
    // ❿ IMPLEMENT ChunkProcessor in Helpers/Utilities.swift
    // ❿ MODIFY loadSummaryContent() TO USE ChunkProcessor
    ...
    private func loadTimelineContent() async throws -> LanguageModelSession {
        ...
        let session = LanguageModelSession(model: model, instructions: {
            ...
            3. Consolidate information from multiple transcripts into a single coherent timeline
            4. Only include information explicitly stated or reasonably inferred from the transcripts
            5. If a date is mentioned relatively (e.g., "next Friday", "in two weeks"), preserve it as stated but mark it as relative
            6. If critical information is missing or unclear, note it in the appropriate field
