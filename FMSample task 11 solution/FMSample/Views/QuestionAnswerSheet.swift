...
struct QuestionAnswerSheet: View {
            ...
            .task {
                if session == nil {
                    ...
                    session = LanguageModelSession(
                        ...
                        Important:
                        - Always use the checkAvailability tool when the user asks about their availability.
                        - If the user's question is unrelated to the provided information, say "Sorry, I can't answer that question".
                        """
                    )
                }
    ...
    private func fetchAnswer() async throws {
        ...
        let stream = session.streamResponse(generating: Response.self) {
            "Question: \(question)\n"
            "This is all the information you can use to answer the question: "
            text
        }
        for try await partialAnswer in stream {
            if partialAnswer.content.insufficientInformation == true {
                answer = "I couldn't find enough information to answer this."
            } else {
                var answerText = AttributedString(partialAnswer.content.answer ?? "")
                answerText.font = .body

                if let citation = partialAnswer.content.citation, !citation.isEmpty {
